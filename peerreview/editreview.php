...
try
{
    ...
    else #There's no reason not to run up the submission interface now
    {
        ...
        $content .= "<form id='review' action='".get_redirect_url("submitreview.php?assignmentid=$assignment->assignmentID$getParams$closeOnDone")."' method='post'>";
		...
		if(array_key_exists("showall",$_GET))
		{
			...
			if($reviews)
			{
				...
				{
					if(!array_key_exists($review->submissionID->id, $reviewMap))
						error_log("DEBUG -- Missing submission: {$review->submissionID->id}");
					if(!array_key_exists($review->reviewerID->id, $reviewMap[$review->submissionID->id]))
						error_log("DEBUG -- Missing review: {$review->submissionID->id}");
					
					if($dataMgr->isStudent($review->reviewerID) && 
					   !$reviewMap[$review->submissionID->id][$review->reviewerID->id]->instructorForced)
					{
				        ...
				        $content .= "</textarea>\n";
						if($reviewMark->markTimestamp) $content .= "<h4>Last Updated: ".date("Y-m-d H:i:s",$reviewMark->markTimestamp)."</h4>";
