...
try
{
    ...
    foreach($displayMap as $authorID => $authorName)
    {
		...
		if ($latestCalibrationAssignment && isIndependent($authorID, $latestCalibrationAssignment))
        ...
        if($submissionID)
        {
            ...
            if(array_key_exists($submissionID->id, $spotCheckMap))
            {
                ...
                $content .= "<tr><td><br><a  target='_blank' href='viewer.php?courseid=$dataMgr->courseID&assignmentid=$assignment->assignmentID&$args&type$i=spotcheck&submissionid$i=$submissionID'>Spot check by ".$displayMap[$spotCheck->checkerID->id]."</a> <b>(".$spotCheck->getStatusString().")</b></td></tr>";
            }
            $content .= "</table>";
        }
