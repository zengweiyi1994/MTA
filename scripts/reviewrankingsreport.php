...
class ReviewRankingsReportScript extends Script
{
    ...
    function executeAndGetResult()
    {
		...
		foreach($dataMgr->getUserDisplayMap() as $authorID => $authorName)
        {
			...
			$student->calibrationScore = (getWeightedAverage(new UserID($authorID), $latestCalibrationAssignment) == "--") ? 0 : (getWeightedAverage(new UserID($authorID), latestCalibrationAssignment()));
			$student->reviewScore = precisionFloat(compute_peer_review_score_for_assignments(new UserID($authorID), $assignments))*100;
		...
		{
				$html .= "<tr><td width='5%'>$i</td><td nowrap>".$student->name."</td>
				<td width='95%'>
				<div style='opacity: 0.5; text-align: right; height: 20px; width: ".($student->calibrationScore*10)."%; background-color: #0080FF;'>".($student->calibrationScore*10)."</div>
